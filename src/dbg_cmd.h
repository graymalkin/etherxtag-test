...
enum dbg_cmd_type {
   ...
   DBG_CMD_ETHERXTAG_LOCK_ACK
};


/**
  It's very helpful if these structures are literally the same as the data 
  arrays they encode. As such we should tell the compiler not to pad values.
*/
#pragma pack(push)
#pragma pack(1)
typedef struct {
...
typedef struct {
  ...
  unsigned int enable_digital_scope;
  unsigned int enable_analog_scope;
  unsigned int data[MAX_DBG_CMD_DATA_LEN-9];
